...
interface ContasAPagarNovoProps {
  selectedEmpresa: string;
}

export function ContasAPagarNovo({ selectedEmpresa }: ContasAPagarNovoProps) {
  ...
  const { data: contas, isLoading } = useQuery({
    queryKey: ["contas-pagar-novo", selectedEmpresa],
    queryFn: async () => {
  ...
  const { data: pagamentosConta } = useQuery({
    queryKey: ["pagamentos-conta", modalPagamentos?.id],
    queryFn: async () => {
  ...
  const mutationPagamento = useMutation({
    ...
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["contas-pagar-novo"] });
  ...
  }, [contas, filtroStatus, filtroTexto, filtroFornecedor, filtroResponsavel, somenteAbertas]);
